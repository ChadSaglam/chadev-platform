...
py_loc=$(find "$dir/backend" -name '*.py' -not -path '*/venv/*' -not -path '*/alembic/versions/*' 2>/dev/null | xargs cat 2>/dev/null | wc -l | tr -d ' ')
ts_loc=$(find "$dir/frontend/src" \( -name '*.ts' -o -name '*.tsx' \) 2>/dev/null | xargs cat 2>/dev/null | wc -l | tr -d ' ')
...
big=$(find "$dir/frontend/src" "$dir/backend/app" \( -name '*.py' -o -name '*.tsx' -o -name '*.ts' \) 2>/dev/null | xargs wc -l 2>/dev/null | grep -v total | sort -rn | head -5 | awk '{printf "  - `%s` %s\n",$2,$1}' | sed "s#$dir/##")
